...
export const ModelWindow = (props) => {
  ...
  return (
    <div style={{display:'inline-block'}}>
      <button className='button-new' onClick={onOpenModal}>{props.buttonText}</button>
          ...
          <div className='custom-model'>
            <h5 style={{textAlign:'left'}} >{props.modelHeading} </h5>
                ...
                <div  style={{alignItems:'center', marginTop:'5px'}}>
                    <button className='button-new' disabled={!title} onClick= {() => { onCloseModal(); addTask(title);}}>{props.modelHeading}</button>    
                </div>    
            </div>
